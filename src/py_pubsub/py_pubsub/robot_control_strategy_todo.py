...
class ControlStrategy(Node):
    ...
    def perform_action_diff_drive_one_step(self):
        if(self.robot_current_pose is not None):
            ...
            # dq = 
            delta_x = v * np.cos(self.robot_current_pose[2]) * self.Ts
            delta_y = v * np.sin(self.robot_current_pose[2]) * self.Ts
            delta_theta = w * self.Ts
            #TODO Update the self.robot_current_pose, i.e., x_{t+1} = x_{t} + self.Ts*dq
            self.robot_current_pose[0] += delta_x
            self.robot_current_pose[1] += delta_y
            self.robot_current_pose[2] += delta_theta
...
def main(args=None):
    ...
    control_strategy.diff_drive_init(left_wheel_v=0.5, right_wheel_v=0.5, duration =10)
    while rclpy.ok():
        try:
